...
layout = [[sg.Button('Terminar Partida', size=(15,1)), sg.Text('Jugador: 100 pts', size=(23, 1), justification='center', font=("Arial Black", 10), relief=sg.RELIEF_RIDGE), sg.Text(size=(10, 1), font=('Helvetica', 15), justification='center', key='-OUTPUT-')],
          ...
          [sg.Button(vector[0], size=(4, 2), key=(vector[0]), pad=(0,0)), sg.Button(vector[1], size=(4, 2), key=(vector[1]), pad=(0,0)), sg.Button(vector[2], size=(4, 2), key=(vector[2]), pad=(0,0)), sg.Button(vector[3], size=(4, 2), key=(vector[3]), pad=(0,0)), sg.Button(vector[4], size=(4, 2), key=(vector[4]), pad=(0,0)), sg.Button(vector[5], size=(4, 2), key=(vector[5]), pad=(0,0)), sg.Button(vector[6], size=(4, 2), key=(vector[6]), pad=(0,0))],
...
while True:                  # the event loop
    ...
    if event == None or counter==0:
        break      
    if event in (range(0,14),range(0,14)):  # Al tener las keys del tablero en tuplas y las keys del atril en
        print (event)                       # strings al comparar en los if nos tira error.
    elif event in Letras:
        print(event)
        keys_entered = event
        windowT[event].update(keys_entered,button_color=('white','blue'))        
